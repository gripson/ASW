...
public static void save(Hotel hotel){
    String hotelid = session.get("hotelid");
    Hotel myhotel = Hotel.find("byId", hotelid).first();

    if(myhotel == null){
        hotel.create();
        Admin admin = Application.connected();
        admin.hotel = hotel;
        admin.save();
        session.put("hotelid", hotel.id);
    }else{
        myhotel.hotelName = hotel.hotelName;
        myhotel.hotelAddress = hotel.hotelAddress;
        myhotel.hotelPrice = hotel.hotelPrice;
        myhotel.details = hotel.details;
        myhotel.fax = hotel.fax;
        myhotel.telephone = hotel.telephone;
        if (hotel.photo!=null){
            myhotel.photo=hotel.photo;
        }
        myhotel.save();
    }
    ...
    public static void photo(){
        Hotel hotel=Hotel.findById(session.get("hotelid"));
        renderBinary(hotel.photo.get());
    }
}
